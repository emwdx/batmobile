...
#pragma config(Sensor, dgtl4,  remoteEnableSet, sensorDigitalIn)
...
#pragma config(Sensor, dgtl9,  remoteEnableSignalLight, sensorDigitalOut)
#pragma config(Sensor, dgtl10, auxiliaryLightingTwo, sensorDigitalOut)
#pragma config(Sensor, dgtl11, driftControlPin, sensorDigitalIn)
...
#pragma config(Sensor, in5,    steeringPot,    sensorPotentiometer)
...
#define THROTTLE_SCALE_FACTOR 25
...
int getStickShiftPosition() {
	...
	else if(1525 > shiftStickValue && shiftStickValue > 1425) {
		// shifter in high
	...
	else if(1775 > shiftStickValue && shiftStickValue > 1675) {
		// shifter in low
...
bool isSystemEnabled(){
	...
	return SensorValue[remoteEnableSet] == 1;
...
void driveControlSchemeKid(int stickShiftPosition, int boostMode, int driftMode) {
	...
	if(isSystemEnabled()){
		...
		SensorValue[remoteEnableSignalLight] = 1;
	}
	else{
		driveMotors(0, boostMode, driftMode, stickShiftPosition);
		SensorValue[remoteEnableSignalLight] = 0;
	}
...
int getDriftMode() {
	//drift - 6D
	int retVal;
if (SensorValue[driftControlPin] == 0){
	//if(vexRT[Btn6D] == 1) {
		retVal = DRIFT_ON;
	}
	else {
